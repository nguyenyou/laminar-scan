(function () {
  ...
  /**
   * Frontend Devtools - Development tools for Frontend applications
   *
   * ✅  DEVTOOLS ARE ENABLED BY DEFAULT ✅
   *
   * Configuration Options:
   *
   * 1. Global Configuration (before script loads):
   *    window.FrontendDevtoolsConfig = {
   *      enabled: true,                     // Enable devtools functionality
   *      preferredIDE: "vscode",           // Set preferred IDE (idea, vscode, cursor, windsurf)
   *      viewportVisibilityFilter: true,   // Filter tree nodes by viewport visibility
   *      initialIsOpen: false              // Auto-open devtools panel on page load
   *    };
   *
   * 2. Runtime Configuration (after script loads):
   *    FrontendDevtools.configure({ enabled: true });
   ...
   *
   * When disabled, devtools have minimal performance impact and do not:
   * - Initialize event listeners
   * - Create overlay or tooltip elements
   ...
   * @property {boolean} enabled - Enable or disable all devtools functionality
  ...
  /**
   * Global configuration object that can be set before script loads
   * @typedef {Object} FrontendDevtoolsConfig
   * @property {boolean} [enabled] - Enable or disable all devtools functionality
   * @property {string} [preferredIDE] - Preferred IDE protocol (idea, vscode, cursor, windsurf)
   * @property {boolean} [viewportVisibilityFilter] - Filter tree nodes based on viewport visibility
   * @property {boolean} [initialIsOpen] - Open devtools panel by default when page loads
   */
  ...
  const DEVTOOLS_PROPERTIES = {
    ...
    /** Scala name property for UIComponent elements (method name) */
    SCALA_NAME: "__scalaname",

    /** Mark as component property for UIComponent elements */
    MARK_AS_COMPONENT: "__markascomponent",
  };
  ...
  const PropertyAccessor = {
    ...
    getSourcePath(element) {
      return /** @type {any} */ (element)[
        DEVTOOLS_PROPERTIES.SCALA_SOURCE_PATH
      ];
    },
    ...
    getSourceLine(element) {
      return /** @type {any} */ (element)[
        DEVTOOLS_PROPERTIES.SCALA_SOURCE_LINE
      ];
    },

    /**
     * Get scala name (method name) from element
     * @memberof PropertyAccessor
     * @param {Element} element - Element to get scala name from
     * @returns {string|undefined} Scala name or undefined
     */
    getScalaName(element) {
      return /** @type {any} */ (element)[DEVTOOLS_PROPERTIES.SCALA_NAME];
    },
     ...
     * @memberof PropertyAccessor
     * @param {Element} element - Element to check
     * @returns {boolean} True if element is marked as component
     */
    isMarkedAsComponent(element) {
      return /** @type {any} */ (element)[DEVTOOLS_PROPERTIES.MARK_AS_COMPONENT] === "true";
    },
  ...
  class PersistentStorage {
    ...
    static getString(key, defaultValue = "") {
      try {
        ...
        console.warn(
          `Error reading string from localStorage key "${key}":`,
          error,
        );
        return defaultValue;
      }
    ...
    static setString(key, value) {
      try {
        ...
        console.warn(
          `Error writing string to localStorage key "${key}":`,
          error,
        );
        return false;
      }
  ...
  /** @type {string} Local storage key for devtools enabled setting */
  const DEVTOOLS_ENABLED_KEY = "devtools_enabled";
  ...
  const PREFER_IDE_PROTOCOL = (() => {
    // Check global configuration first
    if (
      typeof window !== "undefined" &&
      window.FrontendDevtoolsConfig &&
      window.FrontendDevtoolsConfig.preferredIDE
    ) {
      return window.FrontendDevtoolsConfig.preferredIDE;
    }
    return PersistentStorage.getString(PREFER_IDE_KEY, "idea");
  })();
  ...
  const MOUSEMOVE_THROTTLE_DELAY =
    parseInt(
      getComputedStyle(document.documentElement).getPropertyValue(
        "--devtools-throttle-delay",
      ),
    ) || 50;
  ...
  class OverlayManager {
    ...
    handleStateChange(type, data) {
      switch (type) {
        ...
        case "targetElementChanged":
          ...
          if (
            data.element &&
            (!this.state.treeSelectionActive ||
              this.state.treeSelectedElement === data.element)
          ) {
    ...
    show(targetElement) {
      ...
      if (
        this.state.treeSelectionActive &&
        this.state.treeSelectedElement === targetElement
      ) {
        visualState = "active";
      } else if (
        this.state.keyboardNavigationActive &&
        this.state.keyboardSelectedElement === targetElement
      ) {
        visualState = "active";
    ...
    hide() {
      ...
      const transitionDuration =
        this.styleManager.getCSSProperty("transition-normal") || "0.15s";
    ...
    calculatePosition(targetRect) {
      const offset =
        parseInt(this.styleManager.getCSSProperty("element-offset")) || 6;
  ...
  class TooltipManager {
    ...
    /**
     * Get enhanced component display name with method name for marked components
     * @param {Element} targetElement - Target DOM element
     * @param {string} filename - Component filename
     * @returns {string} Enhanced component name (ComponentName#MethodName for marked components)
     */
    getEnhancedComponentDisplayName(targetElement, filename) {
      const componentName = this.getComponentDisplayName(filename);

      if (PropertyAccessor.isMarkedAsComponent(targetElement)) {
        const methodName = PropertyAccessor.getScalaName(targetElement);
        const lineNumber = PropertyAccessor.getSourceLine(targetElement);
        if (methodName) {
          return `${componentName}#${methodName}:${lineNumber}`;
        }
      }
    ...
    showMain(targetElement) {
      ...
      // Build tooltip content using enhanced component name (includes method name for marked components)
      const componentName = this.getEnhancedComponentDisplayName(targetElement, scalafilename);
    ...
    positionMainTooltip() {
      ...
      const targetRect =
        this.state.currentTargetElement.getBoundingClientRect();
      const offset =
        parseInt(this.styleManager.getCSSProperty("element-offset")) || 6;
    ...
    calculateTooltipPosition(overlayRect, tooltipWidth, tooltipHeight) {
      ...
      const viewportHeight = window.innerHeight;
      const margin =
        parseInt(this.styleManager.getCSSProperty("tooltip-margin")) || 10;
  ...
  class ViewportVisibilityDetector {
    ...
    filterVisibleElements(elements) {
      return elements.filter((element) =>
        this.getElementVisibility(element, false),
      );
    }
  ...
  class KeyboardNavigator {
    ...
    navigate(direction) {
      ...
      const currentElement =
        this.state.keyboardSelectedElement || this.state.currentTargetElement;
  ...
  class EventManager {
    ...
    initializeEventListeners() {
      ...
      const debouncedResize = this.createDebouncedHandler(() => {
        if (
          this.devtoolsSystem.state.currentTargetElement &&
          this.devtoolsSystem.state.altPressed
        ) {
          this.devtoolsSystem.state.setCurrentTargetElement(
            this.devtoolsSystem.state.currentTargetElement,
          );
        }
      }, DEBOUNCE_DELAY);
    ...
    handleKeyDown(event) {
      ...
      if (event.key === "Alt" && !this.devtoolsSystem.state.altPressed) {
        ...
        if (
          this.devtoolsSystem.treeView &&
          this.devtoolsSystem.treeView.isVisible
        ) {
        ...
        this.renderDevtoolsOverlay(
          /** @type {MouseEvent} */ (syntheticMouseEvent),
        );

        // Force overlay to show even if target element hasn't changed
        const targetElement = this.getTargetElementAtPosition(
          /** @type {MouseEvent} */ (syntheticMouseEvent),
        );
        if (targetElement) {
    ...
    handleMouseMove(event) {
      ...
      this.devtoolsSystem.state.updateMousePosition(
        event.clientX,
        event.clientY,
      );
    ...
    destroy() {
      this.listeners.forEach((listenerList) => {
        listenerList.forEach(
          (
            /** @type {{target: EventTarget, type: string, handler: Function, options: Object}} */ {
              target,
              type,
              handler,
              options,
  ...
  class ComponentTreeView {
    ...
    hide() {
      ...
      this.animateExit(() => {
        ...
        document.removeEventListener(
          "click",
          this.handleClickOutsideSettings,
          true,
        );
    ...
    createPanel() {
      ...
      title.textContent = "Frontend DevTools";
      title.style.cssText = `
        font-weight: 600;
    ...
    renderTree() {
      ...
      if (
        ...
        this.treeContainer.children.length === 0
      ) {
        const refreshButton = document.createElement("button");
    ...
    renderTreeNode(node, container) {
      ...
      if (this.devtoolsSystem.options.viewportVisibilityFilter) {
        const isVisible = this.viewportDetector.isElementInViewport(
          node.element,
        );
        if (!isVisible) {
      ...
      if (node.children.length > 0) {
        ...
        expandIcon.style.transform = node.expanded
          ? "rotate(90deg)"
          : "rotate(0deg)";
    ...
    animateNodeToggle(node, _wasExpanded) {
      ...
      const expandIcon = nodeElement.querySelector(
        ".devtools-tree-expand-icon",
      );
      if (expandIcon) {
        expandIcon.innerHTML = node.expanded ? "▼" : "▶";
        const htmlExpandIcon = /** @type {HTMLElement} */ (expandIcon);
        htmlExpandIcon.style.transform = node.expanded
          ? "rotate(0deg)"
          : "rotate(-90deg)";
    ...
    handleTreeKeyDown(event) {
      ...
      if (this.selectedNodeId) {
        currentIndex = visibleNodes.findIndex(
    ...
    updateNodeSelection(previousId, currentId) {
      ...
      if (previousId && this.treeContainer) {
        const prevElement = this.treeContainer.querySelector(
          `[data-node-id="${previousId}"]`,
        );
        if (prevElement) {
          ...
          const nameElement = htmlPrevElement.querySelector(
            ".devtools-tree-name",
          );
          if (nameElement) {
      ...
      if (currentId && this.treeContainer) {
        const currentElement = this.treeContainer.querySelector(
          `[data-node-id="${currentId}"]`,
        );
        if (currentElement) {
          const htmlCurrentElement = /** @type {HTMLElement} */ (
            currentElement
          );
          htmlCurrentElement.style.background = "var(--tree-node-selected-bg)";
          htmlCurrentElement.style.boxShadow =
            "0 0 0 1px var(--tree-node-selected-border)";

          // Highlight component name
          const nameElement = htmlCurrentElement.querySelector(
            ".devtools-tree-name",
          );
          if (nameElement) {
            const htmlNameElement = /** @type {HTMLElement} */ (nameElement);
    ...
    handleTreeItemClick(event) {
      ...
      const nodeElement = /** @type {Element} */ (event.target).closest(
        "[data-node-id]",
      );
    ...
    clearAllCaches() {
      ...
      if (
        this.devtoolsSystem.overlay &&
        this.devtoolsSystem.overlay.styleManager
      ) {
        this.devtoolsSystem.overlay.styleManager.clearCache();
      }

      if (
        this.devtoolsSystem.tooltip &&
        this.devtoolsSystem.tooltip.styleManager
      ) {
    ...
    showRefreshFeedback() {
      ...
      const refreshIcon = this.refreshButton.querySelector(
        ".devtools-tree-refresh-icon",
      );
    ...
    addDragAndResizeListeners() {
      ...
      const resizeHandles = this.panelElement.querySelectorAll(
        ".devtools-tree-resize-handle",
      );
    ...
    handleResize(event) {
      ...
      if (direction.includes("top") && newHeight === minHeight) {
        newTop =
          this.resizeState.startTop + this.resizeState.startHeight - minHeight;
      }
    ...
    handleEdgeCases() {
      ...
      this.forEachNode(this.treeData, (node) => {
        if (!document.contains(node.element)) {
          console.warn(
            "ComponentTreeView: Component element no longer in DOM",
            node,
          );
        }
      });
    ...
    createSettingsPanel() {
      ...
      // Create enable devtools setting (master toggle)
      const enableDevtoolsContainer = document.createElement("div");
      enableDevtoolsContainer.style.cssText = `
        display: flex;
        align-items: center;
        justify-content: space-between;
        margin-bottom: 12px;
      ...
      const enableDevtoolsLabel = document.createElement("label");
      enableDevtoolsLabel.textContent = "Enable Devtools";
      enableDevtoolsLabel.title =
        "Enable or disable all devtools functionality (requires page reload)";
      enableDevtoolsLabel.style.cssText = `
        color: var(--tree-text-color);
        font-size: 12px;
        cursor: pointer;
        flex: 1;
      ...
      const enableDevtoolsToggle = document.createElement("input");
      enableDevtoolsToggle.type = "checkbox";
      enableDevtoolsToggle.checked = getDevtoolsEnabled();
      enableDevtoolsToggle.style.cssText = `
        margin-left: 8px;
        cursor: pointer;
        appearance: auto;
      `;

      // Add change handler for enable devtools toggle
      enableDevtoolsToggle.addEventListener("change", (e) => {
        ...
        this.showSettingsFeedback(
          enabled
            ? "Devtools enabled! Reload the page to take effect."
            : "Devtools disabled! Reload the page to take effect.",
        );
      });

      enableDevtoolsLabel.appendChild(enableDevtoolsToggle);
      enableDevtoolsContainer.appendChild(enableDevtoolsLabel);
      this.settingsPanel.appendChild(enableDevtoolsContainer);
      ...
      ideSelectionLabel.title =
        "Select your preferred IDE for opening source files";
      ...
      ideSelectionDropdown.addEventListener("change", (e) => {
        ...
        if (
          EDITOR_PROTOCOL[
            /** @type {keyof typeof EDITOR_PROTOCOL} */ (selectedIDE)
          ]
        ) {
      ...
      initialIsOpenLabel.title =
        "Automatically open the devtools panel when the page loads";
    ...
    /**
     * Show feedback message for settings changes
    ...
    showSettingsFeedback(message) {
      ...
      let feedbackElement =
        this.settingsPanel?.querySelector(".settings-feedback");

      if (!feedbackElement) {
        feedbackElement = document.createElement("div");
        feedbackElement.className = "settings-feedback";
        ...
        if (this.settingsPanel) {
          this.settingsPanel.appendChild(feedbackElement);
        }
      }
    ...
    startViewportMonitoring() {
      ...
      this.throttledViewportRefresh = this.throttle(() => {
        if (
          this.isVisible &&
          this.devtoolsSystem.options.viewportVisibilityFilter
        ) {
      ...
      window.addEventListener(
        "resize",
        /** @type {EventListener} */ (this.throttledViewportRefresh),
      );
    }
  ...
  class DevtoolsSystem {
    ...
    constructor(options = {}) {
      ...
      // Early exit if devtools are disabled
      if (!this.options.enabled) {
        console.log("🚫 Frontend Devtools disabled via configuration");
        return;
      }
    ...
    getStatus() {
      ...
      return {
        ...
        currentTarget: this.state.currentTargetElement
          ? {
              filename: PropertyAccessor.getFilename(
                this.state.currentTargetElement,
              ),
              line: PropertyAccessor.getSourceLine(
                this.state.currentTargetElement,
              ),
              path: PropertyAccessor.getSourcePath(
                this.state.currentTargetElement,
              ),
            }
          : null,
    ...
    navigateToComponent(filename, line) {
      ...
      const target = components.find(
        (comp) => comp.filename === filename && comp.line === line,
      );
    ...
    importConfig(config) {
      ...
      if (
        config.preferredIDE &&
        EDITOR_PROTOCOL[
          /** @type {keyof typeof EDITOR_PROTOCOL} */ (config.preferredIDE)
        ]
      ) {
      ...
      if (typeof config.viewportVisibilityFilter === "boolean") {
        PersistentStorage.setString(
          VIEWPORT_VISIBILITY_FILTER_KEY,
          config.viewportVisibilityFilter.toString(),
        );
  ...
  /**
   * Extract React component display name from a DOM element
   * Optimized specifically for React 19.1.1
   *
   * @param {HTMLElement} domElement - DOM element rendered by React
   * @returns {string|null} Component display name or null if not found
   */
  function getComponentDisplayName(domElement) {
    if (!domElement || !domElement.nodeType) {
      return null;
    }

    // React 19.1.1 uses __reactFiber$ + random suffix for internal keys
    const fiber = getFiberFromElement(domElement);
    if (!fiber) {
      return null;
    }

    // Find the component fiber that rendered this element
    const componentFiber = findComponentFiber(fiber);
    if (!componentFiber) {
      return null;
    }

    // Extract display name from the component fiber
    return extractDisplayName(componentFiber);
  }

  /**
   * Get Fiber node from DOM element (React 19.1.1 specific)
   * @private
   */
  function getFiberFromElement(element) {
    // React 19.1.1 uses __reactFiber$ + random key pattern
    for (const key of Object.keys(element)) {
      if (key.startsWith("__reactFiber$")) {
        const fiber = element[key];
        if (fiber && typeof fiber === "object" && fiber.tag !== undefined) {
          return fiber;
        }
      }
    }
  ...
  /**
   * Find component fiber by walking up the tree
   * @private
   */
  ...
  /**
   * Check if fiber represents a React component (React 19.1.1 tags)
   * @private
   */
  function isComponentFiber(fiber) {
    // React 19.1.1 component work tags
    const tag = fiber.tag;
    return (
  ...
  /**
   * Extract display name from component fiber
   * @private
   */
  function extractDisplayName(fiber) {
    ...
    // Memo component (React.memo)
    if (type.$$typeof && type.type) {
      const wrappedType = type.type;
      return (
        type.displayName ||
        (typeof wrappedType === "function"
          ? wrappedType.displayName || wrappedType.name
          : null) ||
        "Memo"
      );
    }
  ...
  /**
   * Find the nearest parent element with Scala source path information or React component
   * @param {Element} startElement - Element to start searching from
   * @returns {Element|null} Element with source path property or React component, or null
   */
  function findDevtoolsElement(startElement) {
    let element = /** @type {Element|null} */ (startElement);

    // Traverse up the DOM tree to find an element with source path property or React component
    while (element && element !== document.body) {
      // First check for Laminar component (prioritize Laminar)
      if (PropertyAccessor.hasSourcePath(element)) {
        return element;
      }

      // If not a Laminar component, check for React component
      const reactComponentName = getComponentDisplayName(element);
      if (reactComponentName && reactComponentName.endsWith(".scala")) {
        // Set the sourcePath to the React component name for React components
        /** @type {any} */ (element)[DEVTOOLS_PROPERTIES.SCALA_SOURCE_PATH] =
          reactComponentName;
        // Extract clean filename from potential file path and remove extensions
        let cleanFilename = reactComponentName;
        // Handle file paths - extract just the filename portion
        ...
        // Remove file extensions (including React-specific ones)
        cleanFilename = cleanFilename.replace(/\.(scala|js|jsx|ts|tsx)$/, "");

        /** @type {any} */ (element)[DEVTOOLS_PROPERTIES.SCALA_FILENAME] =
          cleanFilename;
        /** @type {any} */ (element)[DEVTOOLS_PROPERTIES.SCALA_SOURCE_LINE] =
          "1";
        return element;
      }
  ...
  function getViewportVisibilityFilter() {
    ...
    if (
      typeof window !== "undefined" &&
      window.FrontendDevtoolsConfig &&
      typeof window.FrontendDevtoolsConfig.viewportVisibilityFilter ===
        "boolean"
    ) {
      return window.FrontendDevtoolsConfig.viewportVisibilityFilter;
    }

    const stored = PersistentStorage.getString(
      VIEWPORT_VISIBILITY_FILTER_KEY,
      "false",
    );
    return stored === "true";
  ...
  function getInitialIsOpen() {
    ...
    if (
      typeof window !== "undefined" &&
      window.FrontendDevtoolsConfig &&
      typeof window.FrontendDevtoolsConfig.initialIsOpen === "boolean"
    ) {
      return window.FrontendDevtoolsConfig.initialIsOpen;
    }
  ...
  /**
   * Get the configured devtools enabled setting
   * @returns {boolean} Whether devtools functionality is enabled
   */
  function getDevtoolsEnabled() {
    // Check global configuration first (highest priority)
    if (
      typeof window !== "undefined" &&
      window.FrontendDevtoolsConfig &&
      typeof window.FrontendDevtoolsConfig.enabled === "boolean"
    ) {
      return window.FrontendDevtoolsConfig.enabled;
    }

    // Fall back to localStorage (default to true - enabled by default)
    const stored = PersistentStorage.getString(DEVTOOLS_ENABLED_KEY, "true");
    return stored === "true";
  }

  /**
   * Set the devtools enabled setting
   * @param {boolean} enabled - Whether to enable devtools functionality
   * @returns {void}
   */
  function setDevtoolsEnabled(enabled) {
    PersistentStorage.setString(DEVTOOLS_ENABLED_KEY, enabled.toString());
  }
  ...
  /**
   * Global API for configuring Frontend Devtools
   * @namespace FrontendDevtools
   */
  window.FrontendDevtools = {
    /**
    ...
    enable() {
      ...
      console.log("✅ Frontend Devtools enabled");
    },
    ...
    disable() {
      ...
      console.log("🚫 Frontend Devtools disabled");
    },

    /**
     * Check if devtools are enabled
     * @returns {boolean} True if devtools are enabled
     */
    isEnabled() {
  ...
  function initializeDevtoolsSystem() {
    // Check if devtools are disabled before initialization
    ...
    try {
      ...
      console.log("✅ Frontend Devtools initialized");
    } catch (error) {
