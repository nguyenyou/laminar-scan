  ...
  private _handlePointerDown(e: PointerEvent) {
    ...
    const handlePointerMove = (moveEvent: PointerEvent) => {
      ...
      rafId = requestAnimationFrame(() => {
        ...
        if (
          !hasMoved &&
          (Math.abs(deltaX) > DRAG_CONFIG.thresholds.dragStart ||
            Math.abs(deltaY) > DRAG_CONFIG.thresholds.dragStart)
        ) {
          hasMoved = true;
          this.setAttribute("dragging", "");
          this.dispatchEvent(
            new CustomEvent("drag-start", { bubbles: true, composed: true })
          );
        }
    ...
    const handlePointerEnd = () => {
      ...
      this.dispatchEvent(
        new CustomEvent("drag-end", { bubbles: true, composed: true })
      );
